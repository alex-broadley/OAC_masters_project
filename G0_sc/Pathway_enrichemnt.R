...
table(tum_categories$Cycle_type)

tumor_scran_counts <- epi_counts[,colnames(epi_counts) %in% rownames(tum_categories)]

tum_counts = tumor_scran_counts
...
#scale enrichment scores for better heatmap plot
GO_enrichment.scores <- scale(GO_enrichment.scores)
...
#enrich_metadata$Cycle_type = str_to_sentence(enrich_metadata$G0_class)
