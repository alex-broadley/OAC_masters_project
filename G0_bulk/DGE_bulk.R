...
# creates a plot displaying the standard deviation across 
...
#use size factors to get normalized counts
...
SdPlot_normalised <- create_SDplot(counts_normalized, "non-transformed normalised data")
SdPlot_log2 <- create_SDplot(counts_log_normalized, "log2 transformation with pseudocount")
...
#change row and column names to display sample type on rows and gender 
#on columns on the heatmap
rownames(sampleDistMatrix_vsd) <- vsd$sample_type
colnames(sampleDistMatrix_vsd) <- vsd$gender
...
for (row in 1:nrow(volcano_df)) {
  if (volcano_df$Gene[row] %in% head(volcano_df[order(volcano_df$padj), "Gene"], 100)){
    print(volcano_df$Gene[row])
...
bulk_enrich_bar = ggplot(data = od[0:10,], aes(x = Term_Description, y = Fold_Enrichment, fill = Fold_Enrichment)) +
  geom_bar(stat = 'identity') +
  coord_flip() +
  labs(x = "", y = "Fold Enrichment", title = "KEGG pathway enrichment (top 10 pathways)") +
  theme_classic() +
  scale_fill_gradient(low = '#b3908b', high = 'darkred', (name = 'Fold Enrichment')) + 
