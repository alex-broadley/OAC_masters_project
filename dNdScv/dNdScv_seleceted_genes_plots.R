...
# plotting points by mutation types with dN/dS > 1
most_selected_type_plot <- function(dfname, point_size, font_size, Title) {
  ...
  for (i in 1:nrow(dfname)) {
    ...
    dfname$most_selected_score[i] = max(c(dfname$miss[i], dfname$non[i], dfname$splice[i]))
  ...
  plot = ggplot(dfname, aes(x = most_selected_score, y = minusLogQall, label = gene_name, color = most_selected_type)) +
    geom_text_repel(max.overlaps = 20, size = font_size, force = 10) +
    geom_point() +
    ...
    labs(x = "Highest MLE dN/dS ratio across mutation types", 
         ...
         title = Title) +
    scale_colour_manual(values=c("#00BB38", "#619DFF", "grey"))
...
candidate_geneType_plot <- function(dfname, point_size, font_size, Title) {
  ...
  for (i in 1:nrow(dfname)) {
    #if positive selection on nonesense or splice mutations, label as candidate tumour suppressor
    if (dfname$nonsense[i] > 1 | dfname$splice[i] > 1){
      dfname$gene_type[i] = 'tumour suppressor'
    }
...
candidate_geneType_plot(signif_slow_genes, 1, 12, "Slowly proliferating tumours")
candidate_geneType_plot(signif_quick_genes, 1, 12, "Aggressively proliferating tumours")
...
mSelected_quick = most_selected_type_plot(signif_quick_genes, 0.8, 5,  "Aggressively proliferating")
mSelected_slow = most_selected_type_plot(signif_slow_genes, 0.8, 5,  "Slowly proliferating")
...
## Primary tumours prevalence mutation type plot
...
## Metastatic tumours prevalence mutation type plot
...
#Mutation type plots for slow and aggressively dividing tumours
figure = grid.arrange(mSelected_quick, mSelected_slow, ncol = 2)
annotate_figure(figure, top = text_grob("Genes and mutation types under positive selection in aggressively and slowly proliferating tumours", size = 20, face = "bold"))
